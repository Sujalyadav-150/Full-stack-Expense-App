...
(async () => {
  ...
  try {
    ...
    // Craft an expired token payload
    const expiredPayload = `expired-id.${testUser.email}.${Date.now() - 60000}.salt`;
    const hmac = crypto.createHmac('sha256', `expense-tracker-secure-reset-salt-secret-key:${crypto.scryptSync('newPasswordCase3', 'expense-tracker-salt', 64).toString('hex')}`);
    hmac.update(expiredPayload);
    const expiredToken = `${Buffer.from(expiredPayload).toString('base64url')}.${hmac.digest('hex')}`;
    ...
    console.log('\nAll 8 automated test cases passed successfully!');
  } finally {
    server.close();
  }
})();
